...
sanitazise_title = function(str) {
  str = str.replace(document.location.host, '');
  str = str.replace(document.location.host.toUpperCase(), '');
  str = str.replace(String('www' + document.location.host).toUpperCase(), '');
  return str;
};
...
domReady((function() {
  return chrome.extension.sendRequest({
    ...
    if (allowedSites.length > 0) {
      f = function(b) {
        if (b) {
          setTimeout('!init()console.log("saving...");', 2000);
        }
        return b;
      };
      e = function(b) {
        var s, site, _i, _j, _len, _len2;
        for (_i = 0, _len = allowedSites.length; _i < _len; _i++) {
          s = allowedSites[_i];
          console.log('?is', s, host);
        }
        for (_j = 0, _len2 = allowedSites.length; _j < _len2; _j++) {
          site = allowedSites[_j];
          f(site === host || host === String('www.' + site + ''));
        }
        return b;
      };
      e();
